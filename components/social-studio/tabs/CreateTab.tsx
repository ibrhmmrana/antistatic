        ...
        for (const platform of selectedChannels) {
          try {
            if (platform === 'google_business') {
              ...
              // Add media if available (use first media item - image or video)
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-6">
        <div className="bg-white rounded-lg border border-slate-200 p-6 shadow-sm">
          ...
          {/* GBP Video Warning */}
          {selectedChannels.includes('google_business') && 
           uploadedMedia.length > 0 && 
           (uploadedMedia[0].type === 'video' || uploadedMedia[0].url.match(/\.(mp4|webm|ogg|mov|avi|mkv)(\?|$)/i)) && (
            <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-2">
              <svg className="w-5 h-5 text-amber-600 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
              </svg>
              <p className="text-sm text-amber-800">
                <strong>GBP video support is experimental.</strong> Try MP4 format and keep file size small for best results.
              </p>
            </div>
          )}
