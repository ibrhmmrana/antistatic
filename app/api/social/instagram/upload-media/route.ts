...
/**
 * POST /api/social/instagram/upload-media
 * 
 * Upload image/video to Supabase Storage for Instagram publishing
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const fileName = `instagram/${businessLocationId}/${timestamp}-${randomUUID()}.${fileExtension}`
    const filePath = fileName

    // Ensure the bucket exists and is public
    // Note: This assumes the Storage bucket exists and has public access
    ...
    if (uploadError) {
      console.error('[Instagram Upload] Storage error:', uploadError)
      return NextResponse.json({ 
