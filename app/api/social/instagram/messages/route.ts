  ...
  try {
    ...
    const formattedConversations = Object.entries(conversationsMap).map(([participantId, conversationEvents]) => {
      ...
      return {
        ...
        displayName: cachedUser?.name || `user_${participantId.slice(-6)}`,
        avatarUrl: cachedUser?.profile_pic_url || null,
        ...
        messages: sortedEvents.map((e: any) => {
          ...
          return {
            ...
            displayName: senderCache?.name || `user_${e.sender_id?.slice(-6) || 'unknown'}`,
            avatarUrl: senderCache?.profile_pic_url || null,
            text: e.text || '',
